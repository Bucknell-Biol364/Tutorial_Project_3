...
To narrow analysis, we can further look at the number of COVID-19 deaths per year and month in Pennsylvania. The state of Pennsylvania was chosen for epidemiological relevance. Furthermore, refining the dataset to a single state is beneficial for improved data visualization. The graphs above yielded general distributions of the dataframe, but limited information can be collected from them. 


Data collection from Pennsylvania can be filtered from the "Covid_19" dataframe. This can be completed using the highly useful "filter" command. The structure of the filter command is as follows: new dataframe name<-filter(old dataframe name, column==/!=/>=/<= "Info from column")
...
After runnning this new code chuck, the new dataframe "PennData" was created from the previous "Covis_19" dataframe, and is present in the "Global Environment". In this dataframe, one will notice 1020 observations and 16 variables. Our columns remained the same, however, all the observations are only from the state of Pennsylvania. Furthermore, notice how "State=="Pennsylvania"" command kept data from that one particular state. Additionally, note how != removed "All Sexes" observations from the Sex column. These observations were removed because they would have been repetitive data as Male+Female obdervations=All sexes observations. 
...
This dataframe can be further refined using the filter function to explore COVID-19 deaths by Age, Group, and Year. Lets also make a graph. 

FIRST, lets use the filter function and narrow down the "PennData" dataframe further. Lets create a new dataframe called "CovidDataPennTotal".We will be using the same structure as before. Did you notice anything about the age group column? There are many observations that are repetitive data. By taking a moment, one can think about that data one might actually want. There are both "All Ages" and "Under 1 year" categories. Furthermore, there are age categories that are grouped by two different age ranges. We should trying to analyze age groups as cohesively as possible. 

A small problem arises with this analysis. The adolescent observations collected data at a different age intervals that are not consistent with the way the adult age intervals collected. Thus, one is forced to look at age range 0-17 for children, followed by a consistent 10 year interval range for adults. 
...
The filter command can be used to removed grouped data (i.e Age.Group!="All Ages") and can be used to remove undesired age ranges. Furthermore, the "Group=="By Total" should be applied to use inclusive grouped data that is comprised of all time data collected from 2020 to 2022 across our desired age ranges. 

SECOND, in this same group of code, we can plot a graph for data visualization. The following steps take place which extend upon the steps used in part 1 of this data visualization section:
  1. Create a graph name and graph using the ggplot package 
    ...
    b. The structure of a basic plot is as possible ggplot(Dataframe, aes(x=column, y= column))
      -Unless specified, R will choose the best fir plot with the given data (it might not be the desired plot you want)
...
One additional line of code, however, should be included to change the x axis numeric categories (which represent the numberic month) to a character (month name). This can be applished using the alread loaded package "Stat2Data". The command structure is as follows: scale_x_continuous(breaks = seq_along(month.name), labels = month.name). This line changes the month observation of "2" to display as February on the X axis.
...
Finally, lets plot COVID-19 Deaths from Pennsylvania by year to see if we can improve on our intial plots from section 1. 
...
One can use the dpylr package to mutate their dataframe. This package is already preloaded into our RMD. Dplyr uses the operators %>% to pipeline commands. This is a close equivalent to useing +.
...
This, almost filted data, should be replaced into dataframe (new or old)-> dataframe.
...
Colors can be represented numerous ways in R. However, they can be conveniently picked by utilizing this provided respurce from Columbia University.
...
4. Code for Visualizing "do older people have higher death rates than younger people?" (BT)
...
5. Code for Ranking net Deaths by State (BT)
