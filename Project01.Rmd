...
###When I ran all of the code the chunk below had a problem with it. The sum(Covid_19.S.T.X.A)one was not found (MK comment)
Checking if the total deaths match for each time division (by month, by year, total)

```{r}
sum(Covid_19.S.T.X.A$COVID.19.Deaths)
# All US total deaths (from the 50 states) from "by total" time category
...
After running this new code chuck, the new dataframe "PennData" was created from the previous "Covid_19" dataframe, and is present in the "Global Environment"/Enviroment tab. In this dataframe, one will notice 1020 observations and 16 variables. Our columns remained the same, however, all the observations are only from the state of Pennsylvania. Furthermore, notice how "State=="Pennsylvania"" command kept data from that one particular state. Additionally, note how != removed "All Sexes" observations from the Sex column. These observations were removed because they would have been repetitive data as Male+Female observations=All sexes observations. 
...
### I think we should probably show these problems in a basic non-fancy graph, cause rn as someone who didnt do that to see it I dont really have the context to understand it. We could also consider breaking the filter and visualization into two chunks so its easier to follow (BT comment)
