...
The effects I explained above will show up once you have knitted the document. The text wont appear italicised or bolded etc when you run your code, but when the document has been finalized the text will be altered accordingly.
...
This dataframe can be further refined using the filter function to explore COVID-19 deaths by Age, Group, and Year similarly to as was previously done. Note that the filtered "PennData" dataframe is now being saved as "CovidDataPennTotal", and that the number of observations in the dataframe is decreasing as we continue to focus in on certain data. 
